...
function GetCounterChartData() {
    return new Promise(function (resolve, reject) {
        ...
        $.ajax({
            type: "POST",
            url: '/Distributor/Mail/GetCounterChartData',
...
function OnLoadDistributorChart() {
    GetCounterChartData().then(function success(result) {
        debugger;
        if(result && result.success && result.data)
            BuildDistributorChart(result.data);
...
function BuildDistributorChart(barItems) {
    ...
    if (mailDistributorChart)
        mailDistributorChart.destroy();


    mailDistributorChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labelsArr,
            datasets: [{
...
if (refreshBtn)
    refreshBtn.addEventListener("click", OnLoadDistributorChart, false);

OnLoadDistributorChart();